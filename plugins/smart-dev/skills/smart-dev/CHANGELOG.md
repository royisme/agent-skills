...
- Track B semantic check now uses the `semantic-checker` agent (no external API key).
- `check-semantic.ts` validates existing semantic-check.json output (optional).
- Documentation updated to remove ANTHROPIC_API_KEY references.
